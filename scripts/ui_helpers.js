function setMainContentUI() {
    ...
    style.ui_contentMarginLeft = (
        100 -
        parseInt(style.ui_contentWidth) -
        parseInt(style.ui_windowLeftRightMargin)
    ).toString();
    ...
    const tocWidthPercent =
        100 -
        parseInt(style.ui_contentWidth) -
        parseInt(style.ui_windowLeftRightMargin) * 2 -
        parseInt(style.ui_gapWidth);
    const tocWidth =
        vw(100) <= maxWidth
            ? tocWidthPercent
            : (tocWidthPercent * maxWidth) / vw(100);
    style.ui_tocWidth = tocWidth.toString();

    const paginationLeftPercent =
        parseInt(style.ui_contentWidth) / 2 +
        parseInt(style.ui_contentMarginLeft);
    const paginationLeft =
        vw(100) <= maxWidth
            ? paginationLeftPercent
            : (((paginationLeftPercent * maxWidth) / 100 +
                  (vw(100) - maxWidth) / 2) /
                  vw(100)) *
              100;
    ...
    if (isVariableDefined(tocContainer)) {
        ...
        tocContainer.style.left = `calc(${
            contentContainer.getBoundingClientRect().x
        }px - ${tocContainer.getBoundingClientRect().width}px - ${
            style.ui_gapWidth
        }% - ${style.ui_windowLeftRightMargin}%)`;
    }
    ...
    if (isVariableDefined(progressContainer)) {
        ...
        progressContainer.style.left = `calc(${
            contentContainer.getBoundingClientRect().x
        }px - ${tocContainer.getBoundingClientRect().width}px - ${
            style.ui_gapWidth
        }% - ${style.ui_windowLeftRightMargin}%)`;
        ...
        if (!isIncreasing) {
            if (
                paginationContainer.offsetWidth >
                    contentContainer.offsetWidth * 0.5 &&
                parseInt(style.ui_numPaginationItems) > 5
            ) {
                style.ui_numPaginationItems = (
                    parseInt(style.ui_numPaginationItems) - 2
                ).toString();
                style.ui_numPaginationItems = Math.max(
                    parseInt(style.ui_numPaginationItems),
                    5
                ).toString();
                generatePagination();
            }
                ...
                paginationContainer.offsetWidth +
                    2 *
                        (paginationContainer.offsetWidth /
                            (parseInt(style.ui_numPaginationItems) + 2)) <
                    contentContainer.offsetWidth * 0.5 &&
                parseInt(style.ui_numPaginationItems) < 9
            ) {
                style.ui_numPaginationItems = (
                    parseInt(style.ui_numPaginationItems) + 2
                ).toString();
                style.ui_numPaginationItems = Math.min(
                    parseInt(style.ui_numPaginationItems),
                    9
                ).toString();
                generatePagination();
...
function setMainContentUI_onRatio() {
    if (window.innerWidth < window.innerHeight) {
        ...
        progressContainer.style.paddingLeft =
            style.ui_windowLeftRightMargin + "%";
...
function showDropZone(focused = false) {
    ...
    if (
        isVariableDefined(dropZone) &&
        isVariableDefined(dropZoneText) &&
        isVariableDefined(dropZoneImg)
    ) {
        let c = null;
        ...
        if (isVariableDefined(bookshelf)) bookshelf.hideTriggerBtn();
        return 0;
    } else {
        return 1;
    }
...
function hideDropZone() {
    if (
        ...
        // Fix icons location with/without scrollbar
        style.ui_btnOffset =
            (-1 * parseInt(style.ui_scrollBarWidth)).toString() + "px";

        // Show bookshelf trigger button if a book is opened
        if (isVariableDefined(bookshelf)) bookshelf.showTriggerBtn();
    }
}
