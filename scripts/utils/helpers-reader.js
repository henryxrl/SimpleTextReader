 ...
 *
 * @module utils/helpers-reader
 * @requires config/index
 * @requires modules/text/pagination
 * @requires utils/base
 */

import * as CONFIG from "../config/index.js";
import { PaginationCalculator } from "../modules/text/pagination-calculator.js";
...
export async function setChapterTitleActive(titleID) {
    ...
    const event = await waitForTocRendered();
    // console.log("ToC Rendered Event Triggered:", event.detail);
...
async function waitForTocRendered() {
    return new Promise((resolve) => {
        ...
        document.addEventListener(
            "tocRendered",
            (event) => {
                resolve(event); // Resolve the promise when the event is dispatched
            },
