...
import { handleSelectedFile } from "../../utils/helpers-file.js";
import {
...
class BookshelfDB {
     ...
     * @async
     * @param {string} name - Book filename
     * @returns {Promise<[File, boolean]|null>} Book data and server status, or null if not found
     * @throws {Error} When database initialization fails
     */
    async getBook(name) {
        if (!(await this.init())) {
...
const bookshelf = {
        ...
        if (this.enabled) {
            ...
            try {
                ...
                if (!(await this.isBookExist(fname))) {
                    ...
                    // let toProcessBookInfo = CONFIG.VARS.ALL_BOOKS_INFO.find(x => x.name === fname);
                    let toProcessBookInfo = CONFIG.VARS.ALL_BOOKS_INFO[fname];
    ...
    genBookItem(bookInfo, idx) {
        ...
        let canvasWidth = getSizePrecise(CONFIG.RUNTIME_VARS.STYLE.ui_bookCoverWidth);
        let canvasHeight = getSizePrecise(CONFIG.RUNTIME_VARS.STYLE.ui_bookCoverHeight);
        let currentBookNameAndAuthor = TextProcessor.getBookNameAndAuthor(removeFileExtension(bookInfo.name));
        ...
        book.find(".coverContainer")
            ...
            .on("mouseleave", function (e) {
                if (
                    ...
                    let tempBookAuthor = TextProcessor.getBookNameAndAuthor(removeFileExtension(bookInfo.name));
        ...
        // }).on("mouseleave", function() {
        //     let tempBookAuthor = TextProcessor.getBookNameAndAuthor(bookInfo.name.replace(/(.txt)$/i, ''));
        //     if (tempBookAuthor.author === "") {
        //         book.find(".coverContainer").css('box-shadow', 'var(--ui_bookshadow_noAuthor)');
        //     } else {
        //         book.find(".coverContainer").css('box-shadow', 'var(--ui_bookshadow)');
        ...
        if (this.enabled) {
            ...
            try {
                ...
                for (const [idx, bookname] of allBooksInfo_names.entries()) {
                    let bookInfo = CONFIG.VARS.ALL_BOOKS_INFO[bookname];
    ...
    enable() {
        if (!this.enabled) {
            ...
            document.addEventListener("handleMultipleBooksWithoutLoading", async (event) => {
                const { fileList, isFromLocal, isOnServer } = event.detail;
                if (this.enabled) {
                    // showLoadingScreen();
                    ...
                    try {
                        ...
                        for (const [i, file] of fileList.entries()) {
                            ...
                            setIsFromLocal(file.name, final_isFromLocal);
                            setIsOnServer(file.name, final_isOnServer);
                            // await this.saveBook(file, final_isFromLocal, final_isOnServer, (i === fileList.length - 1), false);
            ...
            $(".booklist").bind("contentchange", function () {
                ...
                let windowHeight = $(CONFIG.DOM_ELEMENT.DROPZONE).outerHeight(true);
     ...
     */
    init() {
        $(`<div id="STRe-bookshelf-btn" class="btn-icon hasTitle" title="${CONFIG.RUNTIME_VARS.STYLE.ui_tooltip_goToBookshelf}">
