...
export class TitlePatternDetector {
    ...
    static detectTitlePatternAsRegexRule(lines, consoleLog = true) {
        ...
        this.#log("Selected best patterns", this.bestPatterns);
        ...
        try {
            ...
            this.#log("Constructed best pattern regex", bestPatternRegexStr);
            if (consoleLog) {
            ...
            this.#log("Error constructing regex from pattern", this.bestPatterns, error);
        }
    }
    ...
    static #detectPatterns(lines) {
        ...
        lines.forEach((line, index) => {
            ...
            this.#log("Detected pattern with placeholder", { line, patternWithPlaceholder });
    ...
    static #calculatePatternScore({ frequency, isSequential, pattern, examples, numbers }) {
        ...
        // Calculate the actual pattern length excluding this.#TOKEN_NUMBER
        const patternLength = pattern.replace(this.#TOKEN_NUMBER, "").length;
    ...
    static #isPotentialTitleLine(patternWithPlaceholder) {
        ...
        if (prefix === "") {
            if (suffixStartsWithPuncOrSpaceOrEmpty) {
                this.#log('[isPotentialTitleLine][Prefix ""] Potential title line', {
                    patternWithPlaceholder,
                    suffixStartsWithPuncOrSpaceOrEmpty,
                });
                return true;
            } else {
                this.#log('[isPotentialTitleLine][Prefix ""] Invalid potential title line', {
                    patternWithPlaceholder,
                    suffixStartsWithPuncOrSpaceOrEmpty,
                });
                return false;
        ...
        if (this.#titleWords.some((word) => prefix.startsWith(word))) {
            if (suffixStartsWithPuncOrSpaceOrEmpty || suffixStartsWithValidChineseThenPuncOrSpaceOrEmpty) {
                this.#log("[2] Full word matching succeeded", {
                ...
                this.#log("[2] Invalid potential title line", {
                    patternWithPlaceholder,
                    suffixStartsWithPuncOrSpaceOrEmpty,
                    suffixStartsWithValidChinese,
                    suffixStartsWithValidChineseThenPuncOrSpaceOrEmpty,
        ...
        this.#log("[2] Full word matching failed");
        ...
        if (chineseChars) {
            if (chineseChars.every((char) => this.#titleWords.some((word) => word.startsWith(char)))) {
                ...
                if (suffixStartsWithPuncOrSpaceOrEmpty || suffixStartsWithValidChineseThenPuncOrSpaceOrEmpty) {
                    this.#log("[3] Single character matching succeeded", {
                    ...
                    this.#log("[3] Invalid potential title line", {
                        patternWithPlaceholder,
                        suffixStartsWithValidChinese,
                        suffixStartsWithValidChineseThenPuncOrSpaceOrEmpty,
                    });
        ...
        this.#log("[3] Chinese character matching failed");
    ...
    static #findLongestCommonPattern(pattern1, pattern2) {
        ...
        if (commonPattern.endsWith(this.#TOKEN_NUMBER)) {
            console.log("[1111111]", {
                ...
                pattern1,
                pattern2,
                moreContent: i < pattern1.length || i < pattern2.length,
            });
