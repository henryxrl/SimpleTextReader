 ...
 * // Returns: [{pattern: "第{n}章 ", score: 0.8, count: 3, ...}]
 * ```
 *
 * @module TitlePatternDetector
...
export class TitlePatternDetector {
     ...
     */
    static #LOG_PREFIX = "[TitlePatternDetector";
    static #DEBUG = true;
    ...
    /**
     * Best pattern, its number type and regex string
     * @type {string}
     * @public
     */
    static bestPattern = "";
    static bestPatternNumberType = "";
    static bestPatternRegexStr = "";
     ...
     * @public
     */
    static detectTitlePatternAsRegexRule(lines) {
        // Step 1: Detect valid patterns
        this.validTitlePatterns = this.#detectPatterns(lines);

        if (!this.validTitlePatterns || this.validTitlePatterns.length === 0) {
            this.#log("No valid chapter title patterns detected");
            return;
        }

        this.#log("Detected chapter title patterns", this.validTitlePatterns);

        // Step 2: Select the most likely valid pattern
        this.bestPattern = this.validTitlePatterns[0]?.pattern;
        this.bestPatternNumberType = this.validTitlePatterns[0]?.numberType ?? "arabic";
        this.#log("Selected best pattern", this.bestPattern);
        this.#log("Selected best pattern escaped", this.#escapeRegExp(this.bestPattern));
        this.#log("Selected best pattern number type", this.bestPatternNumberType);
        this.#log("Number pattern", this.#numberPatternDict[this.bestPatternNumberType]);

        if (!this.bestPattern) {
            this.#log("No valid pattern found to construct regex");
            return;
        }

        // Step 3: Construct regex rule
        try {
            const escapedPattern = this.#escapeRegExp(this.bestPattern).replace(
                "\\{n\\}",
                this.#numberPatternDict[this.bestPatternNumberType]
            );

            // Replace placeholder `{n}` with a pattern to match numbers, then create a regex
            this.bestPatternRegexStr = `^\\s*(${escapedPattern})\\s*(.{0,${this.#maxLineLength}})$`;

            // Log the constructed regex
            this.#log("Constructed best pattern regex", this.bestPatternRegexStr);

            // Update custom title rules
            updateTitleRules([this.bestPatternRegexStr]);
        } catch (error) {
            this.#log("Error constructing regex from pattern", this.bestPattern, error);
        }
    }
    ...
    static #detectPatterns(lines) {
        ...
        lines.forEach((line, index) => {
            ...
            // const { value: parsedNumber, type: numberType } = this.#parseChapterNumber(number);
            ...
            // Replace the number with a placeholder {n}
            const patternWithPlaceholder = line.replace(number, "{n}");
            this.#log("Detected pattern with placeholder", { patternWithPlaceholder });
        ...
        this.#log("Detected lines", numberedLines);
        ...
        for (let i = 0; i < numberedLines.length; i++) {
            ...
            for (let j = 0; j < numberedLines.length; j++) {
                ...
                const commonPattern = this.#findLongestCommonPattern(data1.pattern, data2.pattern);
                if (!commonPattern) continue;

                if (!patternGroups.has(commonPattern)) {
                    patternGroups.set(commonPattern, {
                        count: 0,
                        examples: new Set(),
                        numbers: new Set(),
                        numberTypes: new Set(),
                ...
                const group = patternGroups.get(commonPattern);
                if (!group.lineIndices.has(data1.index)) {
                    group.count++;
                    group.lineIndices.add(data1.index);
                }
                group.examples.add(data1.original);
                group.numbers.add(data1.number);
                group.numberTypes.add(data1.numberType);
        ...
        const validPatterns = [];
        patternGroups.forEach((data, pattern) => {
            this.#log("Evaluating pattern", pattern);

            // First, ensure the pattern contains {n}
            if (!pattern.includes("{n}")) return;
            // Ensure the pattern is a valid title pattern
            if (!this.#isValidTitlePattern(pattern)) return;
            this.#log("Pattern is valid");

            // Group by number type
            const patternsByType = new Map();
            data.originalData.forEach((item) => {
                if (!patternsByType.has(item.numberType)) {
                    patternsByType.set(item.numberType, {
                        examples: [],
                        numbers: [],
                    });
                }
                const typeGroup = patternsByType.get(item.numberType);
                typeGroup.examples.push(item.example);
                typeGroup.numbers.push(item.number);
            });

            // Evaluate each type separately
            patternsByType.forEach((typeGroup, numberType) => {
                ...
                this.#log(`Count for ${numberType}`, { actualCount, examples: typeGroup.examples });
        ...
        return validPatterns.sort((a, b) => b.score - a.score);
    }
    ...
    static #calculatePatternScore({ frequency, isSequential, pattern, examples, numbers }) {
        ...
        // Calculate the actual pattern length excluding {n}
        const patternLength = pattern.replace("{n}", "").length;
    ...
    static #isPotentialTitleLine(patternWithPlaceholder) {
        // Split the line by {n}
        const parts = patternWithPlaceholder.split("{n}");
        if (parts.length !== 2) return false;

        // Get meaningful characters
        // .replace(this.#puncPattern, "").trim()
        ...
        const suffixStartsWithValidChineseThenPuncOrSpaceOrEmpty =
            suffixStartsWithValidChinese && suffix.length > 1 ? this.#puncPatternWithSpace.test(suffix[1]) : false;

        // If the line starts with {n}, it is a potential title line
        if (prefix === "") {
            if (suffixStartsWithPuncOrSpaceOrEmpty) {
                this.#log("Potential title line", {
                    patternWithPlaceholder,
                    suffixStartsWithPuncOrSpaceOrEmpty,
                });
                return true;
            } else {
                this.#log("Invalid potential title line", {
                    patternWithPlaceholder,
                    suffixStartsWithPuncOrSpaceOrEmpty,
                });
                return false;
        ...
        if (this.#titleWords.some((word) => prefix.startsWith(word))) {
            if (suffixStartsWithPuncOrSpaceOrEmpty || suffixStartsWithValidChineseThenPuncOrSpaceOrEmpty) {
                this.#log("Full word matching succeeded", {
        ...
        this.#log("Full word matching failed");
        ...
        if (chineseChars) {
            if (chineseChars.every((char) => this.#titleWords.some((word) => word.startsWith(char)))) {
                ...
                if (suffixStartsWithPuncOrSpaceOrEmpty || suffixStartsWithValidChineseThenPuncOrSpaceOrEmpty) {
                    this.#log("Single character matching succeeded", {
        ...
        this.#log("Chinese character matching failed");
    ...
    static #isValidTitlePattern(pattern) {
        // If the pattern only contains {n} and spaces, it is valid
        const strippedPattern = pattern.replace("{n}", "").trim();
        if (!strippedPattern) return true;

        // Check if the non-space characters in the pattern are in the title vocabulary
        const meaningfulChars = strippedPattern.replace(this.#puncPattern, "");
        if (!meaningfulChars.trim()) return true; // If there is no content after removing all spaces and parentheses, it is also valid
        // this.#log("Meaningful characters", meaningfulChars);

        // Convert meaningfulChars to lowercase for comparison
        const lowerMeaningfulChars = meaningfulChars.toLowerCase();
        // this.#log("Lower meaningful characters", lowerMeaningfulChars);

        // 1. First, try full word matching
        if (this.#titleWords.some((word) => lowerMeaningfulChars.includes(word.toLowerCase()))) {
            // this.#log("Full word matching succeeded", {
            //     word: this.#titleWords.find((word) => lowerMeaningfulChars.includes(word.toLowerCase())),
            // });
            return true;
        }
        // this.#log("Full word matching failed");
        ...
        // 2. If full word matching fails, try single character matching for Chinese characters
        const chineseChars = lowerMeaningfulChars.match(/[\u4e00-\u9fff]/g);
        if (chineseChars) {
            return chineseChars.every((char) => this.#titleWords.some((word) => word.includes(char)));
        }
        // this.#log("Chinese character matching failed");
    ...
    static #findLongestCommonPattern(pattern1, pattern2) {
        ...
        // If the common pattern does not contain {n} or is too short, it is invalid
        if (!commonPattern.includes("{n}") || commonPattern.length < 3) {
            return null;
        }

        // If the pattern ends with {n}, it is a valid pattern
        if (commonPattern.endsWith("{n}")) {
            return commonPattern;
        }

        // Ensure the pattern ends with a meaningful separator
        const validEndings = [" ", "·", "。", ".", ":", "：", "—", "-", "_"];
        const lastChar = commonPattern[commonPattern.length - 1];

        if (!validEndings.includes(lastChar)) {
            // Roll back to the last valid separator
            let foundValidEnding = false;
            for (let j = commonPattern.length - 1; j >= 0; j--) {
                if (validEndings.includes(commonPattern[j])) {
                    // If a separator is found, check if the characters before it are a valid title pattern
                    const beforeSeparator = commonPattern.substring(0, j).replace("{n}", "").trim();
    ...
    static #detectPatternsOriginal(lines) {
        ...
        lines.forEach((line) => {
            ...
            this.#log("Original Line:", line);
            this.#log("Simplified Line:", simplified);
        ...
        this.#log("All Detected Patterns:", patterns);
        ...
        return Object.entries(patterns)
            .filter(([pattern, count]) => {
                ...
                this.#log("Pattern:", pattern, "Count:", count, "Valid:", isValid);
    ...
    static #log(message, data = null, error = null) {
        if (!TitlePatternDetector.#DEBUG) return;

        const stack = new Error().stack;
        const callerLine = stack.split("\n")[2];
        const match = callerLine.match(/:(\d+):\d+\)?$/);
        const lineNumber = match ? match[1] : "unknown";
        const prefix = `${TitlePatternDetector.#LOG_PREFIX}: ${lineNumber}]`;
