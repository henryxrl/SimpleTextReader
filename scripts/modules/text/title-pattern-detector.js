...
export class TitlePatternDetector {
    ...
    static #isPotentialTitleLine(patternWithPlaceholder) {
        ...
        if (this.#titleWords.some((word) => prefix.startsWith(word))) {
            if (suffixStartsWithPuncOrSpaceOrEmpty || suffixStartsWithValidChineseThenPuncOrSpaceOrEmpty) {
                this.#log("[isPotentialTitleLine][StartsWithValidChineseWord] Full word matching succeeded", {
                ...
                this.#log("[isPotentialTitleLine][StartsWithValidChineseWord] Invalid potential title line", {
                    patternWithPlaceholder,
                    suffixStartsWithPuncOrSpaceOrEmpty,
                    suffixStartsWithValidChinese,
                    suffixStartsWithValidChineseThenPuncOrSpaceOrEmpty,
        ...
        this.#log("[isPotentialTitleLine][StartsWithValidChineseWord] Full word matching failed");
        ...
        if (chineseChars) {
            if (chineseChars.every((char) => this.#titleWords.some((word) => word.startsWith(char)))) {
                ...
                if (suffixStartsWithPuncOrSpaceOrEmpty || suffixStartsWithValidChineseThenPuncOrSpaceOrEmpty) {
                    this.#log("[isPotentialTitleLine] Single character matching succeeded", {
                    ...
                    this.#log("[isPotentialTitleLine] Invalid potential title line", {
                        patternWithPlaceholder,
                        suffixStartsWithValidChinese,
                        suffixStartsWithValidChineseThenPuncOrSpaceOrEmpty,
                    });
        ...
        this.#log("[isPotentialTitleLine] Chinese character matching failed");
    ...
    static #isValidTitlePattern(pattern) {
        ...
        if (!strippedPattern) {
            this.#log("[isValidTitlePattern][strippedPattern] Valid pattern", {
                pattern,
                strippedPattern,
            });
            return true;
        ...
        if (!meaningfulChars.trim()) {
            ...
            this.#log("[isValidTitlePattern][meaningfulChars] Valid pattern", {
                pattern,
                meaningfulChars,
            });
            return true;
        ...
        if (prefix === "") {
            if (suffixStartsWithPuncOrSpaceOrEmpty) {
                ...
                if (suffixStripped) {
                    this.#log('[isValidTitlePattern][Prefix ""][suffixStartsWithPuncOrSpaceOrEmpty] Invalid pattern', {
                        pattern,
                        suffixStartsWithPuncOrSpaceOrEmpty,
                        suffixStripped,
                    });
                    ...
                    this.#log('[isValidTitlePattern][Prefix ""][suffixStartsWithPuncOrSpaceOrEmpty] Valid pattern', {
                        pattern,
                        suffixStartsWithPuncOrSpaceOrEmpty,
                    });
                    return true;
                ...
                this.#log('[isValidTitlePattern][Prefix ""] Invalid pattern', {
                    pattern,
                    suffixStartsWithPuncOrSpaceOrEmpty,
                    suffixStartsWithValidChinese,
                    suffixStartsWithValidChineseThenPuncOrSpaceOrEmpty,
        ...
        if (this.#titleWords.some((word) => prefix.startsWith(word))) {
            if (suffixStartsWithPuncOrSpaceOrEmpty) {
                ...
                this.#log("[isValidTitlePattern][StartsWithValidChineseWord] Invalid pattern", {
                    pattern,
                    suffixStartsWithPuncOrSpaceOrEmpty,
                    suffixStartsWithValidChinese,
                    suffixStartsWithValidChineseThenPuncOrSpaceOrEmpty,
        ...
        this.#log("[isValidTitlePattern][StartsWithValidChineseWord] Full word matching failed");
        ...
        if (chineseChars) {
            if (chineseChars.every((char) => this.#titleWords.some((word) => word.startsWith(char)))) {
                // If the first character of the suffix is in the titleWordsChineseSuffix, it is a valid title line
                if (suffixStartsWithPuncOrSpaceOrEmpty || suffixStartsWithValidChineseThenPuncOrSpaceOrEmpty) {
                    this.#log("[isValidTitlePattern] Single character matching succeeded", {
                        char: chineseChars.every((char) => this.#titleWords.some((word) => word.startsWith(char))),
                        pattern,
                        suffixStartsWithValidChinese,
                        suffixStartsWithValidChineseThenPuncOrSpaceOrEmpty,
                    ...
                    this.#log("[isValidTitlePattern] Invalid potential title line", {
                        pattern,
                        suffixStartsWithValidChinese,
                        suffixStartsWithValidChineseThenPuncOrSpaceOrEmpty,
                    });
        ...
        this.#log("[isValidTitlePattern] Chinese character matching failed");
