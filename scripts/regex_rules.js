...
const regex_number =
    "一二两三四五六七八九十○零百千壹贰叁肆伍陆柒捌玖拾佰仟0-9０-９";
const regex_titles_chinese_pre_1 = "([第序终終卷\\[【]\\s*[";
const regex_titles_chinese_post_1 = "\\s/\\.、]*\\s*[章节節回集卷部篇季\\]】])";
const regex_titles_chinese_1 =
    regex_titles_chinese_pre_1 + regex_number + regex_titles_chinese_post_1;

const regex_titles_chinese_pre_2 = "([\\[【序终終卷]\\s*[";
const regex_titles_chinese_post_2 = "\\s/\\.、\\]】]*)\\s*";
const regex_titles_chinese_2 =
    regex_titles_chinese_pre_2 + regex_number + regex_titles_chinese_post_2;

const regex_other_titles =
...
const regex_titles_english =
    "chapter|part|appendix|appendices|preface|Foreword|Introduction|Prologue|Epigraph|Table of contents|Epilogue|Afterword|Conclusion|Glossary|Acknowledgments|Bibliography|Index|Errata|Colophon|abstract|introduction";

var rules_titles = [
    `^(\\s*(正文\\s*)?[${regex_number.slice(
        0,
        -6
    )}]*\\s*$)|(\\s*[\\[【](正文\\s*)?[${regex_number.slice(
        0,
        -6
    )}]*[\\]】]\\s*$)`,
    `^(\\s*(正文\\s*)?${regex_titles_chinese_1}\\s*$)|^(\\s*[\\[【](正文\\s*)?${regex_titles_chinese_1}[\\]】]\\s*$)`,
    `^(\\s*(正文\\s*)?${regex_titles_chinese_1}[:：\\s]\\s*(.+)$)|^(\\s*[\\[【](正文\\s*)?${regex_titles_chinese_1}[:：\\s]\\s*(.+)[\\]】]$)`,
    `^(\\s*(正文\\s*)?${regex_titles_chinese_2}\\s*$)|^(\\s*[\\[【](正文\\s*)?${regex_titles_chinese_2}[\\]】]\\s*$)`,
    `^(\\s*(正文\\s*)?${regex_titles_chinese_2}[:：\\s]\\s*(.+)$)|^(\\s*[\\[【](正文\\s*)?${regex_titles_chinese_2}[:：\\s]\\s*(.+)[\\]】]\\s*$)`,
    `^(\\s*(${regex_other_titles})[:：]?\\s*$)|^(\\s*[\\[【](${regex_other_titles})[\\]】][:：]?\\s*$)`,
    `^(\\s*(${regex_other_titles})[:：\\s]\\s*(.+)\\s*$)|(\\s*[\\[【](${regex_other_titles})[:：\\s]\\s*(.+)[\\]】]\\s*$)`,
    `^(\\s*(${regex_titles_english})\\s*$)`,
    `^(\\s*(${regex_titles_english})[:\\s]?\\s*(.+)$)`,
];
...
var rules_zxcs = () => [
    ...
    `^(\\s*((书名)?\\s*${regex_colon}${regex_bracket_left}${bookAndAuthor.bookNameRE}${regex_bracket_right})\\s*([\\(（)]?)((文字精校版)|(校对版全本)|(精校版全本))([)）]?)\\s*((作者)?\\s*${regex_colon}${bookAndAuthor.authorRE})\\s*$)`,
