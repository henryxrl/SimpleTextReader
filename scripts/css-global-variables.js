...
(function () {
    ...
    window.CSSGlobalVariables = function (configObj = {}) {
        ...
        if (!new.target)
            throw new Error(
                'Calling CSSGlobalVariables constructor without "new" is forbidden'
            );
        ...
        if (typeof configObj !== "object")
            throw new Error(
                "CSSGlobalVariables constructor expects a config Object as first argument"
            );

        if (
            configObj.hasOwnProperty("normalize") &&
            typeof configObj.normalize !== "function"
        ) {
            throw new Error('Config property "normalize" must be a function');
        }

        if (
            configObj.hasOwnProperty("autoprefix") &&
            typeof configObj.autoprefix !== "boolean"
        ) {
        ...
        if (configObj.hasOwnProperty("filter")) {
            if (typeof configObj.filter !== "string")
                ...
                try {
                    ...
                    throw new Error(
                        'Provided "filter" is an invalid selector ("' +
                            configObj.filter +
                            '")'
                    );
                }
        ...
        function normalizeVariableName(name = "") {
            ...
            if (name.substring(0, 2) !== "--") {
                if (__config__.autoprefix) name = "--" + name;
                    ...
                    throw new Error(
                        'Invalid CSS Variable name. Name must start with "--" (autoprefix=false)'
                    );
            }
        ...
        function updateVarsCache() {
            ...
            Array.from(document.styleSheets).forEach((_styleSheet) => {
                ...
                if (
                    _styleSheet.ownerNode.getAttribute("css-global-vars-ignore")
                )
                    return;
                ...
                try {
                    ...
                    if (
                        !_styleSheet.ownerNode.hasAttribute(
                            "css-global-vars-ignore"
                        )
                    ) {
                        _styleSheet.ownerNode.setAttribute(
                            "css-global-vars-ignore",
                            true
                        );
                        console.warn(
                        ...
                        console.warn(
                            "Unexpected error reading CSS properties."
                        );
                    }
                    abort = true;
                }
                ...
                let ids =
                    _styleSheet.ownerNode.getAttribute("css-global-vars-id");
                ...
                if (String(ids).split(",").includes(String(__config__.id)))
                    ...
                    // not cached yet!
                    let value =
                        _styleSheet.ownerNode.getAttribute(
                            "css-global-vars-id"
                        );
                ...
                Array.from(_styleSheet.rules || _styleSheet.cssRules).forEach(
                    (cssRule) => {
