...
respectUserLangSetting =
    document.documentElement.getAttribute("respectUserLangSetting") === "true";
webLANG = document.documentElement.getAttribute("webLANG");
console.log(
    `App language is "${webLANG}". Respect user language setting is ${respectUserLangSetting}.`
);
...
const custom_fonts = [
    {
        en: "title",
        zh: "title",
        label_zh: style.ui_font_title_label_zh,
        label_en: style.ui_font_title_label_en,
    },
    {
        en: "body",
        zh: "body",
        label_zh: style.ui_font_body_label_zh,
        label_en: style.ui_font_body_label_en,
    },
    { en: "fzkai", zh: "fzkai", label_zh: "方正楷体", label_en: "FZKaiTi" },
    {
        en: "ui",
        zh: "ui",
        label_zh: style.ui_font_ui_label_zh,
        label_en: style.ui_font_ui_label_en,
    },
    {
        en: "kinghwa",
        zh: "kinghwa",
...
function loadSettings() {
    ...
    light_mainColor_active =
        localStorage.getItem("light_mainColor_active") ||
        light_mainColor_active_default;
    light_mainColor_inactive =
        localStorage.getItem("light_mainColor_inactive") ||
        light_mainColor_inactive_default;
    light_fontColor =
        localStorage.getItem("light_fontColor") || light_fontColor_default;
    light_bgColor =
        localStorage.getItem("light_bgColor") || light_bgColor_default;
    dark_mainColor_active =
        localStorage.getItem("dark_mainColor_active") ||
        dark_mainColor_active_default;
    dark_mainColor_inactive =
        localStorage.getItem("dark_mainColor_inactive") ||
        dark_mainColor_inactive_default;
    dark_fontColor =
        localStorage.getItem("dark_fontColor") || dark_fontColor_default;
    ...
    pagination_bottom =
        localStorage.getItem("pagination_bottom") || pagination_bottom_default;
    pagination_opacity =
        localStorage.getItem("pagination_opacity") ||
        pagination_opacity_default;
...
function loadDefaultSettings() {
    ...
    if (respectUserLangSetting) {
        setSelectorValue(
            "setting_uilanguage",
            Object.keys(ui_language_mapping).indexOf(ui_language)
        );
    }
...
function saveSettings(toSetLanguage = false, forceSetLanguage = false) {
    if (respectUserLangSetting) {
        ui_language =
            $("#setting_uilanguage")
                ...
                .attr("rel") || ui_language_default;
    }
    p_lineHeight =
        $("#setting_p_lineHeight").val() + "em" || p_lineHeight_default;
    p_fontSize = $("#setting_p_fontSize").val() + "em" || p_fontSize_default;
    light_mainColor_active =
        $("#setting_light_mainColor_active").val() ||
        light_mainColor_active_default;
    light_mainColor_inactive = HSLToHex(
        ...hexToHSL(
            $("#setting_light_mainColor_active").val() ||
                light_mainColor_active_default,
            1.5
    ...
    light_fontColor =
        $("#setting_light_fontColor").val() || light_fontColor_default;
    light_bgColor = $("#setting_light_bgColor").val() || light_bgColor_default;
    dark_mainColor_active =
        $("#setting_dark_mainColor_active").val() ||
        dark_mainColor_active_default;
    dark_mainColor_inactive = HSLToHex(
        ...hexToHSL(
            $("#setting_dark_mainColor_active").val() ||
                dark_mainColor_active_default,
            0.5
    ...
    dark_fontColor =
        $("#setting_dark_fontColor").val() || dark_fontColor_default;
    ...
    title_font =
        $("#setting_title_font")
            ...
            .children(".select-options")
            .children(".is-selected")
            .attr("rel") || title_font_default;
    body_font =
        $("#setting_body_font")
            .closest(".select")
            .children(".select-options")
            .children(".is-selected")
            .attr("rel") || body_font_default;
    pagination_bottom =
        $("#setting_pagination_bottom").val() + "px" ||
        pagination_bottom_default;
    pagination_opacity =
        $("#setting_pagination_opacity").val() || pagination_opacity_default;
...
function initiateSettingMenu() {
    ...
    if (respectUserLangSetting) {
        settingUILanguage = createSelectorItem(
            "setting_uilanguage",
            Object.keys(ui_language_mapping),
            Object.values(ui_language_mapping)
        );
    }
    let settingLineHeight = createRangeItem(
        "setting_p_lineHeight",
        parseFloat(p_lineHeight),
        1,
        3,
        0.5,
        (unit = "em")
    );
    let settingFontSize = createRangeItem(
        "setting_p_fontSize",
        parseFloat(p_fontSize),
        1,
        3,
        0.5,
        (unit = "em")
    );
    let settingLightMainColorActive = createColorItem(
        "setting_light_mainColor_active",
        light_mainColor_active,
        (savedValues = ["#2F5086"])
    );
    let settingLightFontColor = createColorItem(
        "setting_light_fontColor",
        light_fontColor,
        (savedValues = ["black"])
    );
    let settingLightBgColor = createColorItem(
        "setting_light_bgColor",
        light_bgColor,
        (savedValues = ["#FDF3DF"])
    );
    let settingDarkMainColorActive = createColorItem(
        "setting_dark_mainColor_active",
        dark_mainColor_active,
        (savedValues = ["#6096BB"])
    );
    let settingDarkFontColor = createColorItem(
        "setting_dark_fontColor",
        dark_fontColor,
        (savedValues = ["#F2E6CE"])
    );
    let settingDarkBgColor = createColorItem(
    ...
    let settingPaginationBottom = createRangeItem(
        "setting_pagination_bottom",
        parseFloat(pagination_bottom),
        1,
        30,
        1,
        (unit = "px")
    );
    let settingPaginationOpacity = createRangeItem(
        "setting_pagination_opacity",
        parseFloat(pagination_opacity),
        0,
        1,
        0.1,
        (unit = "")
    ...
    document.getElementById("setting_p_lineHeight").value =
        parseFloat(p_lineHeight);
    document.getElementById("setting_p_fontSize").value =
        parseFloat(p_fontSize);
    document.getElementById("setting_pagination_bottom").value =
        parseFloat(pagination_bottom);
    document.getElementById("setting_pagination_opacity").value =
        parseFloat(pagination_opacity);
    ...
    selector_init(
        $("#setting_title_font"),
        findFontIndex(title_font),
        saveSettings,
        [false, false]
    );
    selector_init(
        $("#setting_body_font"),
        findFontIndex(body_font),
        saveSettings,
        [false, false]
    );
}
...
function handleClickOutsideBox(event) {
    ...
    isColorPickerOpen.push(
        isVariableDefined(colorPickerArray) && colorPickerArray.length > 0
    );
...
function createSelectorWithGroupItem(
    id,
    values,
    texts,
    groups,
    ...
    fontList.forEach((font) => {
        ...
        if (checkAvailability) {
            ...
            if (
                !fontName &&
                font.macVariant &&
                isFontAvailable(font.macVariant)
            ) {
            ...
            if (
                !fontName &&
                font.linuxVariant &&
                isFontAvailable(font.linuxVariant)
            ) {
...
function changeFontSelectorItemLanguage($selector, lang) {
    ...
    if ($selector && $selector.length) {
        ...
        let $selectedOption = $selector
            .closest(".select")
            .children(".select-styled");
...
function createFontSelectorItem(id) {
    ...
    font_groups = [
        style.ui_font_group_custom_en,
        style.ui_font_group_system_en,
    ];
    font_groups_zh = [
        style.ui_font_group_custom_zh,
        style.ui_font_group_system_zh,
    ];
...
function createRangeItem(id, value, min, max, step, unit = "") {
    ...
    settingItemInputOutput.setAttribute(
        "style",
        "--thumb-text-color:" + style.bgColor + ";"
    );
...
function createColorItem(id, value, savedValues = []) {
    ...
    settingItemInput.setAttribute(
        "style",
        "--color:" + value + "; --colorInverted:" + style.borderColor + ";"
    );
...
function setRangeValue(id, value) {
    ...
    temp_item.parentElement.style.setProperty(
        "--text-value",
