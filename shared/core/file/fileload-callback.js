 ...
 * Credit: cataerogong
 *
 * @module shared/core/file/fileload-callback
 */
...
export const FileLoadCallback = {
    ...
    /** @type {Array<Function>} List of callbacks to execute after database save */
    afterDBSaveList: [],
    ...
    /**
     * Register a callback to be executed after database save
     * @param {Function} callback - Function to be executed after database save
     */
    regAfterDBSave(callback) {
        if (typeof callback == "function" && !this.afterDBSaveList.includes(callback)) {
            this.afterDBSaveList.push(callback);
        }
    },

    /**
     * Unregister an after database save callback
     * @param {Function} callback - Function to be removed from after database save callbacks
     */
    unregAfterDBSave(callback) {
        let i = this.afterDBSaveList.indexOf(callback);
        if (i >= 0) this.afterDBSaveList.splice(i, 1);
    },
    ...
    async before(f) {
        let newF = f;
        try {
            for (const func of this.beforeList) {
    ...
    async after() {
        try {
            for (const func of this.afterList) {
                await func();
            }
        } catch (e) {
    ...
    /**
     * Execute all registered after database save callbacks
     * @returns {Promise<void>}
     */
    async afterDBSave() {
        try {
            for (const func of this.afterDBSaveList) {
                await func();
            }
        } catch (e) {
            console.log("FileLoadCallback.afterDBSave() error:", e);
        }
    },
};
