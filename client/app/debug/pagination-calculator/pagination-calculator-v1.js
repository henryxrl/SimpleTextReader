...
export class PaginationCalculator {
    ...
    /**
     * @private
     * @type {boolean} Whether the book is complete
     */
    #isBookComplete = true;
    ...
    constructor(contentChunks, allTitles, config) {
        ...
        // If the book has only one chapter, avoid smart pagination
        // if (this.#isSingleChapter()) {
        //     this.#useSmartPagination = false;
        // }
    ...
    /**
     * Checks if the book has only one chapter
     * @private
     * @returns {boolean} true if the book has only one chapter
     */
    #isSingleChapter() {
        // If there's only one title (meaning it's either the title page or the end page), then it's a single chapter.
        const condition1 = this.#allTitles.length === 1;
        // Also, if the length is 2, and the first title's text is "扉页" or "TITLE PAGE", and the second title's text is "终" or "THE END", then it's a single chapter.
        // Note: the title text is the first element of the tuple
        const condition2 =
            this.#allTitles.length === 2 &&
            (this.#allTitles[0][0].toLowerCase() === "扉页" || this.#allTitles[0][0].toLowerCase() === "title page") &&
            (this.#allTitles[1][0].toLowerCase() === "终" || this.#allTitles[1][0].toLowerCase() === "the end");
    ...
    #processLongChapter(titleStart, nextTitleStart) {
        ...
        for (let j = titleStart; j < nextTitleStart; j++) {
            const line = this.#contentChunks[j]?.trim() || "";
            contentCount = this.#incrementCount(contentCount, line);
    ...
    /**
     * Increments the count based on the type of pagination
     * @private
     * @param {number} currentCount - Current count
     * @param {string} line - Line of content
     * @returns {number} Updated count
     */
    #incrementCount(currentCount, line) {
        if (this.#useCharCount) {
            return currentCount + (line.trim().length || 0);
        }
        return line.trim() && !this.#isPunctuationOnly(line) ? currentCount + 1 : currentCount;
    }
    ...
    #handleLastChapter() {
        if (!this.#allTitles || this.#allTitles.length === 0) {
            this.#log("No titles found at all");
            return;
        }

        // If only end page exists, process entire content as one chapter
        if (this.#allTitles.length === 1) {
            this.#log("Only end page exists, processing entire content as one chapter");
            const totalLines = this.#contentChunks.length;
            const contentLength = this.#getContentLength(0, totalLines);

            if (contentLength > 0) {
                // 直接处理长章节，让上层处理（file-processor-worker.js）来处理跨块的页面合并
                this.#processLongChapter(0, totalLines);
            }
            return;
        }

        // Process last real chapter the normal case where there are multiple chapters
