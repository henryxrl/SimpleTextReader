 ...
 * @requires client/app/lib/sweetalert2/src/sweetalert2
...
export class PopupManager {
    ...
    static showNotification({ iconName, text, timer = 3000, iconColor = "mainColor_inactive" }) {
        this.#logger.log("[Notification] Starting animation");
        ...
        if (this.#activeNotifications.length >= this.#MAX_NOTIFICATIONS) {
            ...
            this.#logger.log(`[Notification] Notification queued. Queue length: ${this.#notificationQueue.length}`);
            return;
        }
        ...
        setTimeout(() => {
            ...
            this.#logger.log(`[Notification] Animation ended, executing ${this.pendingUIUpdates.size} pending updates`);
    ...
    static #removeNotification({ element, style }) {
        this.#logger.log("[Notification] Starting removal animation");
        this.isAnimating = true;
    ...
    static executePendingUpdates() {
        if (!this.isAnimating && this.pendingUIUpdates.size > 0) {
            this.#logger.log(`[UI Updates] Executing ${this.pendingUIUpdates.size} pending updates`);
            for (const update of this.pendingUIUpdates) {
                update();
            }
            this.#logger.log(`[UI Updates] Executed ${this.pendingUIUpdates.size} pending updates`);
