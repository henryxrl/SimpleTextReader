 ...
 * - Automatically adjust the direction and height of the dropdown menu based on the available space
 *
...
class DropdownSelector {
     ...
     * @param {Array} eventListeners - Array of event listeners with func and params properties
     * @param {Object} options - Additional options for the dropdown selector
     * @param {jQuery} [options.$parent] - jQuery selector object for the parent element
    ...
    #bindEvents() {
        ...
        this.$styledSelect.on("click", (e) => {
            ...
            if (this.$styledSelect.hasClass("active")) {
                ...
                // Get the max height of the dropdown list
                const dropDownStyle = getComputedStyle(this.$list[0]);
                const dropdownHeight = parseInt(dropDownStyle.maxHeight);
                const dropdownStylingHeight =
                    parseInt(dropDownStyle.borderTopWidth) +
                    parseInt(dropDownStyle.borderBottomWidth) +
                    parseInt(dropDownStyle.paddingTop) +
                    parseInt(dropDownStyle.paddingBottom) +
                    2; // 2px for more space
                const dropdownHeightWithBorder = dropdownHeight + dropdownStylingHeight;
                const dropdownMinHeight = parseInt(dropDownStyle.minHeight) + dropdownStylingHeight;

                // Get the boundary of the parent element
                const parentElement = this.options.$parent ? this.options.$parent[0] : document.body;
                const parentRect = parentElement.getBoundingClientRect();
                const selectRect = this.$styledSelect[0].getBoundingClientRect();

                // Calculate the available space within the parent element
                const bottomSpace = parentRect.bottom - selectRect.bottom;
                const topSpace = selectRect.top - parentRect.top;

                // Debugging
                // console.log("Top space:", topSpace);
                // console.log("Bottom space:", bottomSpace);
                // console.log("Dropdown height:", dropdownHeightWithBorder);

                let needShrink = false;
                let finalHeight = dropdownHeightWithBorder;
                const maxAvailableSpace = Math.max(topSpace, bottomSpace);

                // Check if the dropdown needs to be shrunk
                if (bottomSpace < dropdownHeightWithBorder && topSpace < dropdownHeightWithBorder) {
                    needShrink = true;
                    // Calculate the new height (ensure at least one option is visible)
                    finalHeight = Math.max(maxAvailableSpace - dropdownStylingHeight, dropdownMinHeight);
                }

                // Set menu's direction and height
                if (needShrink) {
                    this.$list.css({
                    ...
                    if (topSpace > bottomSpace) {
                        this.$list.addClass("dropup").css({
                            top: "auto",
                            bottom: parentRect.bottom - selectRect.top + "px",
                        });
                    } else {
                        this.$list.removeClass("dropup").css({
                            top: selectRect.bottom - parentRect.top + "px",
                            bottom: "auto",
                        });
                    }
                } else if (bottomSpace >= dropdownHeightWithBorder) {
                    this.$list.removeClass("dropup").css({
                        top: selectRect.bottom - parentRect.top + "px",
                        bottom: "auto",
                    });
